...
class DatasetOption(object):

    @staticmethod
    def get_default_config(updates=None):
        config = ConfigDict()
        config.type = 'feedback' # feedback, pretrain
        config.text_processor = TextProcessor.get_default_config()
        config.hf_dataset = HumanFeedbackDataset.get_default_config()
        config.pt_dataset = PretrainDataset.get_default_config()
    ...
    def load_dataset(cls, config, tokenizer, **kwargs):
        ...
        if config.type == 'feedback':
            return HumanFeedbackDataset(config.hf_dataset, tokenizer, text_processor, **kwargs)
        elif config.type == 'pretrain':
            return PretrainDataset(config.pt_dataset, tokenizer, text_processor, **kwargs)
        else:
            raise ValueError(f'Unknown dataset type: {config.type}')

    def __init__(self):
        raise ValueError('a static class should not be instantiated.')
